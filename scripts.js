...
let manualMessage = null; // ✅ holds custom message

function tick() {
  if (manualMessage !== null) {
    // if custom message set, just display it
    tw.textContent = manualMessage;
    return; 
  }
...
function resetConfetti() {
  ...
  for (let i = 0; i < 160; i++) {
    pieces.push({
      ...
      vx: -2 + R(4), vy: 1 + R(3),
      r: R(360),
      c: ["#ff6b9a", "#ffd166", "#ff9fb3", "#ff87b7"][~~(R(4))]
    });
  }
}
function draw() {
...
document.getElementById('play').addEventListener('click', () => {
  if (!song.src) {
    alert('No song selected. Replace the audio src in the HTML with a file or a URL to an mp3.');
    return;
  }
  if (song.paused) {
    song.play();
    document.getElementById('play').textContent = 'Pause song';
  } else {
    song.pause();
    document.getElementById('play').textContent = 'Play song';
  }
});
...
document.getElementById('edit').addEventListener('click', () => {
  ...
  if (newMsg !== null && newMsg.trim() !== "") {
    manualMessage = newMsg;   // ✅ stop typewriter and show this instead
    tw.textContent = manualMessage;
  } else {
    manualMessage = null;     // ✅ resume typewriter if empty
    tick();
  }
});

// accessibility: press 'T' triggers surprise
